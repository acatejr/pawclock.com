...
from models import Pet, Owner, CareSession, Base
...
def test_create_owners_with_pets(session: Session):
    ...
    assert len(owner.pets) == len(pets)


def test_create_care_session(session: Session):
    ...
    assert owner.pets[0].id is not None
    ...
    assert care_session.id is not None
    assert care_session.start_date is not None
    assert care_session.end_date is None
    assert care_session.pet_id == owner.pets[0].id
    assert care_session.owner_id == owner.id
